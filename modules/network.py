...
def is_zapret_active():
    """Проверяет, запущен ли Zapret (процесс winws.exe). Всегда перепроверяет."""
...
def stop_zapret():
    """Останавливает Zapret. Возвращает True если успешно."""
    ...
    print("   [Zapret] 🛑 Останавливаем предыдущий Zapret...")

    # Способ 1: service_remove (стандартный Zapret)
    zapret_dir = _find_zapret_dir()
    if zapret_dir:
        service_remove = os.path.join(zapret_dir, "service_remove")
        if os.path.exists(service_remove):
            try:
                import subprocess as sp
                sp.Popen(
                    ["cmd", "/c", service_remove],
                    cwd=zapret_dir,
                ...
                time.sleep(3)
                if not is_zapret_active():
                    print("   [Zapret] ✅ Zapret остановлен через service_remove!")
                    return True
            except:
                pass
    ...
    # Способ 2: taskkill
    try:
        _run(["taskkill", "/F", "/IM", "winws.exe"], timeout=5)
        time.sleep(2)
        if not is_zapret_active():
            print("   [Zapret] ✅ Zapret остановлен через taskkill!")
            return True
    except:
        pass

    print("   [Zapret] ⚠️ Не удалось остановить Zapret.")
    return False
...
def start_zapret():
    """Запускает Zapret. Сначала останавливает предыдущий (мог зависнуть). Возвращает True если успешно."""
    # Сначала останавливаем предыдущий — мог зависнуть
    if is_zapret_active():
        print("   [Zapret] 🔄 Перезапускаем Zapret (предыдущий мог зависнуть)...")
        stop_zapret()
        time.sleep(2)
    else:
        print("   [Zapret] 🔄 Zapret не запущен, запускаем...")
