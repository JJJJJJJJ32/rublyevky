...
def publish_lot(session, game_data, short_description, full_description, payment_message, price):
    try:
        ...
        # Переводы и лимиты — 3 перевода ПАРАЛЛЕЛЬНО (вместо 3 по очереди)
        with ThreadPoolExecutor(max_workers=3) as pool:
            f_summary_en = pool.submit(ai_translate_to_en, short_description)
            f_desc_en = pool.submit(ai_translate_to_en, full_description)
            f_pay_en = pool.submit(ai_translate_to_en, payment_message)
            # Английский summary: одна строка, байтовый лимит
            s_en = enforce_limits(f_summary_en.result(), LIMITS['summary_min'], LIMITS['summary_max'], is_en=True, single_line=True, max_bytes=LIMITS['summary_max_bytes'])
            ...
            f_en = enforce_limits(f_desc_en.result(), LIMITS['description_min_en'], 3000, is_en=True)
            # Русский description: без изменений
            f_ru = full_description
            # Английский payment_msg: минимум 300 символов
            p_en = enforce_limits(f_pay_en.result(), LIMITS.get('payment_msg_min_en', 300), LIMITS['payment_msg_max'], is_en=True)
